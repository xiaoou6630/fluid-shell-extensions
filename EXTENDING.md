...
If you need an English version, please use a browser translator.
...
- **radius**：放置半径，0 表示只在中心放置
- **check_air**：是否只在空气方块上放置
...
- **type**：粒子类型，支持：`bubble`, `smoke`, `effect`, `witch`, `bubble_pop`
- **count**：粒子数量
- **spread**：粒子扩散范围
...
### 为原版水添加效果
在 `config/fluid_shell_extensions/effects/` 目录下创建 `minecraft_water.json` 文件，写入以下内容：
...
保存文件并重启游戏。现在，当你发射装有水的流体炮弹时，它会根据流体量产生不同的效果。
...
### Mod 条件检测
使用 `condition` 字段来检测特定 Mod 是否加载，例如：
...
### 配置不生效
- 检查配置文件的格式是否正确，确保 JSON 语法没有错误。
- 检查流体的注册名是否正确，格式为 `命名空间:路径`。
- 检查配置文件的文件名是否正确，建议为 `命名空间_路径.json`。
- 检查配置文件是否放在正确的目录下：`config/fluid_shell_extensions/effects/`。
- 查看游戏日志，是否有关于配置加载的错误信息。

### 效果执行失败
- 检查配置中的物品、流体或药水效果的注册名是否正确。
- 检查是否满足效果的条件（例如，是否加载了特定的 Mod）。
- 查看游戏日志，是否有关于效果执行的错误信息。
